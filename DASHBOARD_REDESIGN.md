...
**Current:** Bubble chart visualization  
**Target:** Beautiful card-based dashboard (like finance apps you showed)
...
### Mobile-First (375px)
```
[Header: CommodityBubbles + User]
[Live Price Ticker - 4 cards, horizontal scroll]
...
```
[Header]
[Grid of 4 commodity cards - 1 row]
[2-column layout: Sentiment + Projects | Risk Dashboard]
[Full sentiment feed below]
```

---

## 📦 Complete New Dashboard Code

**File:** `frontend/app/dashboard/page.tsx` (Complete Replacement)
...
import { Heart, MessageCircle, Share2, AlertTriangle, TrendingUp, MapPin, Calendar } from 'lucide-react';
...
1. **Portfolio Value Card** — Big, prominent display with trend
2. **Commodity Cards** — Clean, colorful (emoji-based)
3. **Sentiment Feed** — Twitter/Reddit-style posts
4. **Risk Dashboard** — Visual risk score with gauge
5. **Project Timeline** — Infrastructure updates
6. **Bottom Navigation** — Mobile-optimized tabs
...
/* Desktop: 4 columns for prices, 3 for layout */
lg:grid-cols-4
lg:grid-cols-3
```
...
1. **Replace file:**
   ```bash
   cp this_new_code frontend/app/dashboard/page.tsx
   ```
...
3. **Push to Vercel:**
   ```bash
   git add .
   git commit -m "Redesign dashboard with beautiful cards"
...
1. **Copy the code above** to `frontend/app/dashboard/page.tsx`
2. **Test locally** with `npm run dev`
3. **Push to GitHub** → Auto-deploys to Vercel
4. **Done!** New design is live
...
- [ ] Real sentiment data (Twitter API)
